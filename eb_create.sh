...
env_status=`aws elasticbeanstalk describe-environments --environment-names $EB_ENV_NAME --region=eu-west-2 --output text --no-include-deleted`
...
while true; do
	...
	env_status=`aws elasticbeanstalk describe-environments --environment-names $EB_ENV_NAME --region=eu-west-2 --query "Environments[*].Status" --output text --no-include-deleted`
