...
# This script will restore the couchbase from previous backup for given environment, 
 ...
 && python /opt/couchbase/cli/cbrestore '~/restore/$RESTORE_FROM_ENV/' 'http://$TARGET_COUCHBASE_ENV:$TARGET_COUCHBASE_INSTANCE_PORT' --bucket-source=$SOURCE_BUCKET --bucket-destination=$DEST_BUCKET \
...
# echo "2" | test $(wc -l) -gt 0 && echo 'hello' || echo "world"
